...
const SignUp = () => {
  ...
  const handleSubmit = () => {
    ...
    if (displayName.length > 12) {
      setErrors((prev) => ({
        ...prev,
        displayName: 'Display Name must be 12 characters or shorter',
      }));
      formHasErrors = true;
  ...
  return (
    <div className="signUp">
      {accountCreated && <div className="logIn">
        <Alert className="logOutBox" variant="success">
          <Alert.Heading>Success!</Alert.Heading>
          <p>Your account has been created!</p>
          <Button variant="outline-success" onClick = {()=>history.push('/')}>Continue to Home Page</Button>
        </Alert>
      </div>}
      {alreadySignedIn && <div className="logIn">
        <Alert className="logOutBox" variant="danger">
          <Alert.Heading>Already Signed In</Alert.Heading>
          <p>Please sign out of the current account before making a new one.</p>
          <Button variant="outline-danger" onClick = {()=>setAlreadySignedIn(false)}>Back</Button>
        </Alert>
      </div>}
